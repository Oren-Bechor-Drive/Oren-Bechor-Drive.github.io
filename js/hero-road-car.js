...
export function initHeroRoadCar(hero) {
	...
	if (!car?.animate || !road?.getPointAtLength || !window.ResizeObserver)
		return;
	...
	function update() {
		...
		function extend(point, neighbor) {
			...
			return {
				x: point.x + (dx / distance) * 100,
				y: point.y + (dy / distance) * 100,
			};
		}
		...
		for (const frame of frames)
			frame.offset =
				((frame.offset / distance) * driveDuration) / cycleDuration;
