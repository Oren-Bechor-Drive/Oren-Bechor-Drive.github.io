export async function initRoadCarousel(root) {
	...
	for (let number = 1; ; number += 1) {
		const url = new URL(`assets/images/students-pass/${number}.png`, document.baseURI);
		const response = await browserWindow.fetch(url.href, { method: "HEAD", cache: "no-cache" });
		if (response.status === 404) break;
		if (!response.ok || !response.headers.get("Content-Type")?.startsWith("image/")) {
			throw new Error(`Unable to discover student photo ${number}: HTTP ${response.status}`);
		}
	...
	const photos = await Promise.all(sources.map(async (src, index) => {
	...
	for (let index = templates.length - 1; index > 0; index -= 1) {
		...
		[templates[index], templates[randomIndex]] = [templates[randomIndex], templates[index]];
	}
	...
	function updateDuration() {
		const secondsPerCar = Number(browserWindow.getComputedStyle(root).getPropertyValue("--road-seconds-per-car"));
		...
		if (distance > 0 && carStep > 0 && secondsPerCar > 0) {
			root.style.setProperty("--road-loop-duration", `${distance / carStep * secondsPerCar}s`);
		}
	}
