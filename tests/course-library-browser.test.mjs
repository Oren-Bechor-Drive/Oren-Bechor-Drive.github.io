...
test("course headers reserve blank profile space and only link the brand to home", async () => {
	for (const file of ["course.html", ...lessons.map(lesson => lesson.file)]) {
		...
		assert.equal(header.querySelector("a").getAttribute("href"), "index.html");
...
for (const mode of ["enhanced", "disabled", "blocked module"]) {
	test(`learner can open the learning page and jump to any section with ${mode} JavaScript`, { timeout: 20_000 }, async t => {
		...
		for (const width of [1440, 390, 320]) {
			...
			await page.route("**/*", route => mode === "blocked module" && route.request().url().endsWith("/js/course-library.js") ? route.abort() : serveRoadMedia(route));
			await page.goto("http://gallery.test/course.html");
			...
			assert.equal(new URL(page.url()).pathname, "/right-of-way.html");
			...
			for (const lesson of lessons) {
				...
				assert.equal(new URL(page.url()).pathname, "/course.html");
			}
...
for (const failure of ["disabled", "blocked module", "blocked storage", "stale storage"]) {
	test(`topic browsing survives ${failure}`, { timeout: 20_000 }, async t => {
		...
		for (const width of [1440, 320]) {
			...
			await page.route("**/*", route => failure === "blocked module" && route.request().url().endsWith("/js/course-library.js") ? route.abort() : serveRoadMedia(route));
			await page.goto("http://gallery.test/course.html");
			assert.equal(await page.locator(".subject:visible").count(), 7);
