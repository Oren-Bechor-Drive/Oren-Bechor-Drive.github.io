...
import { auditRoadMedia, inspectImage } from '../scripts/road-media-integrity.mjs';
...
test('corrupt and unsupported images become issues while later images are audited', async () => {
  ...
  try {
    ...
    await writeFile(path.join(temporaryRoot, 'corrupt.jpg'), Buffer.from([0xff, 0xd8, 0xff]));
    // A complete 1x1 GIF is readable metadata, but outside the road-media format policy.
    await writeFile(path.join(temporaryRoot, 'unsupported.jpg'), Buffer.from('R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7', 'base64'));
    await writeFile(path.join(temporaryRoot, 'metadata.jpg'), await readFile(path.join(rootDir, 'assets/source-road-000.jpg')));
    await writeFile(path.join(temporaryRoot, 'valid.jpg'), await readFile(path.join(rootDir, 'assets/source-road-000.jpg')));

    const audit = await auditRoadMedia({ rootDir: temporaryRoot });
    assert.deepEqual(audit.assets.map(({ source }) => source), ['metadata.jpg', 'valid.jpg']);
    assert.deepEqual(audit.issues, [
