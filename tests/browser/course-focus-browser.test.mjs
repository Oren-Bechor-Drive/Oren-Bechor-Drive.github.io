...
test("search and answer choices avoid pointer outlines while keeping keyboard focus visible", async t => {
	...
	for (const width of [1440, 390]) {
		...
		const border = await field.evaluate(element => getComputedStyle(element).borderColor);
		...
		assert.notEqual(await field.evaluate(element => getComputedStyle(element).borderColor), border, "search keeps a visible focus border");
		await page.goto("http://gallery.test/course/right-of-way/quizzes/priority/");
		...
		assert.equal(await answer.evaluate(element => getComputedStyle(element).outlineStyle), "none");
		await page.keyboard.press("ArrowDown");
		const keyboardAnswer = page.locator(".quiz-answers label:has(input:focus)");
		assert.equal(await keyboardAnswer.evaluate(element => getComputedStyle(element).outlineStyle), "solid");
		assert.equal(await keyboardAnswer.evaluate(element => getComputedStyle(element).outlineWidth), "3px");
		await page.close();
	}
});
