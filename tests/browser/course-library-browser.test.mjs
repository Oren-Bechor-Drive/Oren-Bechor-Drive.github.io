...
const topicControl = (page, id) => page.locator(`.topic-tab[data-topic="${id}"]:visible, #${id} > summary:visible`);
...
test("course preview preserves the welcome page's seven topic descriptions", async () => {
	...
	const summaries = [...course.querySelectorAll(".subject summary")].map(summary => [summary.querySelector("h3").textContent, summary.querySelector("p").textContent]);
	for (const description of descriptions) assert.deepEqual(summaries.find(([title]) => title === description[0]), description);
	assert.equal(summaries.length, 9);
	assert.equal(course.querySelector("[data-search-status]").textContent, "9 נושאים לבחירה");
...
for (const mode of ["enhanced", "disabled", "blocked module"]) {
	test(`topics stay in a fixed desktop list or compact accordion with ${mode} JavaScript`, { timeout: 20_000 }, async t => {
		...
		for (const width of [1440, 1024, 768, 390, 320]) {
			...
			const controls = desktop ? page.locator(".topic-tab") : page.locator(".subject summary");
			const positions = await controls.evaluateAll(elements => elements.map(el => ({ top: el.offsetTop, height: el.getBoundingClientRect().height })));
			for (const id of await page.locator(".subject").evaluateAll(elements => elements.map(el => el.id))) {
				const control = topicControl(page, id);
				await control.focus();
				await page.keyboard.press("Enter");
				assert.equal(await topicOutline(page, id).isVisible(), true);
				assert.equal(await topicOutline(page, id).textContent(), await page.locator(`#${id} .subject-outline`).textContent(), "the reader preserves the complete authored outline");
				if (desktop) {
					assert.equal(await control.getAttribute("aria-selected"), "true");
					assert.equal(await page.getByRole("tab", { selected: true }).count(), 1);
					assert.deepEqual(await controls.evaluateAll(elements => elements.map(el => ({ top: el.offsetTop, height: el.getBoundingClientRect().height }))), positions, "selecting any topic never reflows the topic list");
					const list = await page.getByRole("tablist").boundingBox();
					const reader = await page.getByRole("tabpanel").boundingBox();
					assert.ok(reader.x + reader.width < list.x, "the reading panel sits to the left of the RTL topic list");
					assert.equal(await page.getByRole("tabpanel").getAttribute("aria-labelledby"), await control.getAttribute("id"));
				} else {
					assert.deepEqual(await page.locator(".subject[open]").evaluateAll(elements => elements.map(el => el.id)), [id]);
					assert.ok(positions.every(({ height }) => height < 120), "closed topics are compact rows");
				}
...
test("desktop keyboard selection, fragment links and resizing preserve the selected subject", { timeout: 20_000 }, async t => {
	const browser = await chromium.launch();
	t.after(() => browser.close());
	...
	assert.equal(await topicOutline(page, "licensing-and-points").isVisible(), true);
	await page.keyboard.press("Home");
	await page.keyboard.press("ArrowDown");
	assert.equal(await topicOutline(page, "signs-and-speed").isVisible(), true);
	await page.keyboard.press("Tab");
	assert.equal(await page.getByRole("tabpanel").evaluate(el => el === document.activeElement), true);
	await page.setViewportSize({ width: 390, height: 844 });
	await page.locator(".subject-list").waitFor({ state: "visible" });
	assert.equal(await topicOutline(page, "signs-and-speed").isVisible(), true);
	assert.equal(await topicControl(page, "signs-and-speed").evaluate(el => el === document.activeElement), true);
	...
	assert.equal(await page.getByRole("tab", { selected: true }).getAttribute("data-topic"), "overtaking");
	await page.getByRole("searchbox").fill("מלווה");
	assert.equal(await topicOutline(page, "licensing-and-points").isVisible(), true);
	await page.getByRole("searchbox").fill("zzz");
	assert.equal(await page.getByRole("tabpanel").isVisible(), false, "no stale outline appears beside an empty result");
...
test("library supports search, reset, arbitrary topic selection and return visits", { timeout: 30_000 }, async t => {
	...
	for (const width of [1440, 390, 320]) {
		...
		for (const [query, id] of [["מלווה", "licensing-and-points"], ["גרירה", "licensing-and-points"], ["תיאוריה", "learning-foundations"], ["סיכום", "learning-foundations"], ["מצמד", "right-of-way"], ["הילוך", "driving-test"]]) {
			await search.fill(query);
			assert.equal(await visibleTopics(page).count(), 1, `${query} finds its topic at ${width}px`);
			assert.equal(await topicControl(page, id).isVisible(), true);
		}
		assert.equal(await page.evaluate(() => localStorage.getItem("oren-course:last-topic")), null, "searching alone does not invent a topic visit");
		...
		await topicOutline(page, "roundabouts").waitFor({ state: "visible" });
		assert.equal(await topicOutline(page, "roundabouts").isVisible(), true);
		assert.equal(await page.locator(".subject-outline:visible").count(), 1, "return links show only the requested outline");
		assert.equal(await search.inputValue(), "");
		assert.equal(await topicControl(page, "roundabouts").evaluate(element => element === document.activeElement), true);
...
for (const failure of ["disabled", "blocked module", "blocked storage", "stale storage"]) {
	test(`topic browsing survives ${failure}`, { timeout: 20_000 }, async t => {
		...
		for (const width of [1440, 320]) {
			...
			await page.goto("http://gallery.test/course/");
			assert.equal(await visibleTopics(page).count(), 9);
			assert.equal(await page.locator("[data-search-status]").innerText(), "9 נושאים לבחירה");
			...
			await page.keyboard.press("Enter");
			assert.equal(await topicOutline(page, "driving-test").isVisible(), true, "test preparation opens without completing earlier topics");
			for (const id of ["learning-foundations", "licensing-and-points"]) {
				await topicControl(page, id).focus();
				await page.keyboard.press("Enter");
				assert.equal(await topicOutline(page, id).isVisible(), true, `${id} opens with ${failure}`);
			}
			assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
