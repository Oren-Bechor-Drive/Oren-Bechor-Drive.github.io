...
const photoNumbers = Object.keys(roadPhotoSources)
	.map(Number)
	.sort((a, b) => a - b);
	...
	test(`progressive loading keeps position and order at ${width}px${late === "resize" ? " after widening during loading" : late === "boundary" ? " at the next loop boundary" : late ? " with reduced motion during a pending append" : ""}`, async (t) => {
		...
		await page.addInitScript(() => {
			...
			HTMLImageElement.prototype.decode = async function () {
				...
				const number = Number(
					this.src.match(/students-pass\/(\d+)\.png$/)?.[1],
				);
		...
		await page.route("**/*", async (route) => {
			...
			if (
				...
				if (
					Number(
						url.pathname.match(
							/\/(?:oren-bachor-students-)?(\d+)(?:-\d+)?\.(?:png|webp)$/,
						)?.[1],
					) === (late === "resize" ? 7 : lastPhoto)
				)
					await gate;
			}
		...
		const minimumInitialCount = await page
			.locator('.road-carousel-group:not([aria-hidden="true"])')
			.evaluate((group) => {
				const step =
				...
				return Math.max(
					...
					Math.ceil(innerWidth / step) + 1,
				);
			});
		if (photoCount <= minimumInitialCount) {
			t.skip(
				"The gallery needs more photos to exercise a later append at this viewport",
			);
			return;
		}
		...
		if (late === "resize") {
			...
			await page.waitForFunction(
				() =>
					document.querySelector("[data-road-carousel]").dataset
						.ready !== "true",
			);
		}
		...
		const leftBefore = await page.evaluate((late) => {
			...
			animation.currentTime = late
				...
				: 4000;
			return document.querySelector(".road-car").getBoundingClientRect()
				.left;
		}, late);
		release();
		let boundaryLeft;
		if (late === true || late === "boundary") {
			await page.waitForFunction(
				(number) => window.decodedPhotoNumbers.includes(number),
				lastPhoto,
			);
			await page.evaluate(
				() =>
							...
							new Promise((resolve, reject) => {
								const timeout = setTimeout(
									() =>
										reject(
											new Error(
												"The road did not reach its next loop boundary",
											),
										),
									3000,
								);
		...
		const state = await page.evaluate(() => ({
			left: document.querySelector(".road-car").getBoundingClientRect()
				.left,
			photos: [
				...document
		...
		if (late === "boundary")
			assert.ok(
				Math.abs(state.left - boundaryLeft) < 1,
				"publishing at the loop boundary preserves the visible cars' position",
			);
		if (!late)
			assert.ok(
		...
		assert.deepEqual(
			state.photos.map((photo) =>
				Number(photo.src.match(/\/(\d+)\.png$/)[1]),
			),
			photoNumbers,
		);
		assert.ok(
		...
		assert.deepEqual(
			failedResponses,
			[],
			"gallery loading must not probe missing files",
		);
		assert.deepEqual(
			consoleErrors,
			[],
			"gallery loading must leave the browser console clean",
		);
		assert.ok(
...
for (const width of [1366, 390]) {
	test(`gallery discovery waits until the road approaches the viewport at ${width}px`, async (t) => {
		...
		await page.route("**/*", async (route) => {
			...
			if (/\/js\/road-(?:carousel|photo-sources)\.js$/.test(path))
				galleryModules.push(path);
		...
		await page.goto("http://gallery.test/");
		await page.waitForFunction(
			() => document.documentElement.dataset.menuEnhanced === "true",
		);
		await page.evaluate(
			() =>
				new Promise((resolve) =>
		...
		assert.ok(
			await gallery.evaluate(
				(root) => root.getBoundingClientRect().top > innerHeight + 300,
			),
		);
		assert.deepEqual(
			metadataRequests,
			[],
			"initial rendering must not discover gallery photos",
		);
		assert.deepEqual(
			galleryModules,
			[],
			"gallery code must wait until the road approaches the viewport",
		);
		assert.ok(
			requestedPhotos.every((number) => number <= 6),
			"only baseline photos may download before approaching the road",
		);
		...
		assert.equal(
			...
			photoCount,
			"returning to the road must not initialize it twice",
		);
	});
}
...
test("gallery initializes when IntersectionObserver is unavailable", async (t) => {
	...
	assert.equal(
		await page.locator("[data-road-carousel]").getAttribute("data-ready"),
		"true",
	);
});
