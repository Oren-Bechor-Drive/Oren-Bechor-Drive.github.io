...
for (const malformed of [false, true]) {
  test(`page navigation works with ${malformed ? 'malformed' : 'valid'} topic markup`, async (t) => {
    ...
    const entryUrl = new URL(`../${entry.getAttribute('src')}`, import.meta.url);
    entryUrl.searchParams.set('case', String(malformed));
    const loading = import(entryUrl.href);
    if (malformed) await assert.rejects(loading, /Missing required topic panel/);
    else await loading;
    ...
    if (!malformed) {
      ...
      card.click();
      assert.equal(document.querySelector('[data-topic-panel-title]').textContent, card.textContent.trim());
    }
  });
}
