...
test("ordinary image sources and responsive candidates are audited on a focused page", async (t) => {
	const rootDir = await fixture(t, {
		"index.html": '<img src="missing.png" srcset="also-missing.png 1w, pixel.png 9w"><picture><source srcset="missing-picture.png 2x"><img src="pixel.png" width="42" height="42"></picture>',
		"pixel.png": png,
	});
...
test("image preloads audit responsive candidates and optional declared MIME", async (t) => {
	const rootDir = await fixture(t, {
		"index.html": '<link rel="preload" as="image" href="pixel.png" imagesrcset="missing.png 1w, pixel.png 3w"><link rel="preload" as="image" imagesrcset="pixel.png 1w">',
		"pixel.png": png,
	});
	const audit = await media.auditRoadMedia({ rootDir });
	assert.deepEqual(audit.issues, [
...
test("ordinary SVGs require valid SVG XML while scaled raster sizes are allowed", async (t) => {
	const rootDir = await fixture(t, {
		"index.html": '<img src="pixel.png" width="42" height="42"><img src="icon.svg" width="24" height="24"><img src="wrong.svg"><img src="broken.svg">',
		"pixel.png": png,
		"icon.svg": '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M0 0"/></svg>',
	...
	assert.ok(audit.issues.some(issue => issue.startsWith("wrong.svg:") && issue.includes("SVG")));
	assert.ok(audit.issues.some(issue => issue.startsWith("broken.svg:") && issue.includes("SVG")));
});

test("site audit discovers authored pages and identifies the page for every finding", async (t) => {
	assert.equal(typeof media.auditSiteMedia, "function", "site-wide audit must be available");
	const rootDir = await fixture(t, {
		"index.html": '<img src="shared.png" width="42" height="42">',
		"course.html": '<img src="course-missing.png">',
		"lessons/lesson.html": '<img src="../shared.png?rev=2#image"><img src="/shared.png"><img src="local.png"><link rel="preload" as="image" href="missing-preload.png" type="image/png">',
		"shared.png": png,
	...
	assert.deepEqual(audit.pages, ["course.html", "index.html", "lessons/lesson.html"]);
	assert.deepEqual(audit.issues, [
		"course.html: course-missing.png: file does not exist",
		"lessons/lesson.html: missing-preload.png: file does not exist",
	]);
	assert.ok(audit.assets.some(asset => asset.htmlPath === "index.html" && asset.source === "shared.png"));
	assert.ok(audit.assets.some(asset => asset.htmlPath === "lessons/lesson.html" && asset.source === "local.png"));
});

test("ordinary external and embedded references are skipped without fetching", async (t) => {
	const rootDir = await fixture(t, {
		"index.html": '<img src="https://example.invalid/image.png"><img src="//example.invalid/image.png"><img src="data:image/png;base64,AAAA" srcset="data:image/png;base64,AAAA 1x, local.png 2x"><img src="image%20name.png">',
		"local.png": png,
		"image name.png": png,
	});
	const audit = await media.auditRoadMedia({ rootDir });
...
test("responsive-only preload MIME declarations are checked against their candidates", async (t) => {
	const rootDir = await fixture(t, {
		"index.html": '<link rel="preload" as="image" imagesrcset="pixel.png 1w" type="image/webp">',
		"pixel.png": png,
	});
	const audit = await media.auditRoadMedia({ rootDir });
	assert.deepEqual(audit.issues, ["pixel.png: preload type is image/webp, expected image/png"]);
});

for (const [name, srcset] of [["empty", ""], ["whitespace", " \t "], ["commas", " , ,, \t,"]]) {
	for (const element of ["image", "preload"]) {
		test(`${element} without a source rejects ${name} responsive candidates`, async (t) => {
			const rootDir = await fixture(t, {
				"index.html": element === "image"
					? `<img srcset="${srcset}">`
...
test("marked images still require an original source alongside delivery candidates", async (t) => {
	const rootDir = await fixture(t, {
		"index.html": '<img data-road-media srcset="pixel.png 1w" width="1" height="1" alt="image">',
		"pixel.png": png,
	});
	const audit = await media.auditRoadMedia({ rootDir });
	assert.ok(audit.issues.some(issue => issue.includes("source is empty")));
...
test("media CLI fails on a broken ordinary image outside the welcome page", async (t) => {
	...
	const script = fileURLToPath(new URL("../../scripts/road-media-integrity.mjs", import.meta.url));
	await assert.rejects(promisify(execFile)(process.execPath, [script], { cwd: rootDir }), error => {
		assert.equal(error.code, 1);
		assert.match(error.stderr, /quiz\.html: missing\.png: file does not exist/);
		return true;
	});
});
