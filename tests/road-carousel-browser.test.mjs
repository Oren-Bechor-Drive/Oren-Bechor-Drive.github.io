...
test("real gallery styles keep animation working across viewport changes", { timeout: 30000 }, async (t) => {
	...
	await t.test("loading wheel covers the stopped road until discovery finishes", async () => {
		...
		const state = await loader.evaluate(element => {
			...
			return {
				...
				roadAnimations: document.querySelector(".road-carousel-track").getAnimations().length,
			};
		});
		...
		assert.equal(await loader.locator("img").evaluate(wheel => wheel.getAnimations().length), 0);
	...
	for (const [name, width, height, secondsPerCar] of [
		...
		await t.test(name, async () => {
			...
			assert.equal(state.insideHero, true, "the gallery must belong to the hero");
			assert.ok(state.roadBottom <= height + 1, "the complete road must fit in the initial viewport");
			assert.ok(Math.abs(state.heroBottom - state.roadBottom) < 1, "the road must finish the hero");
			assert.ok(state.actionsBottom <= state.roadTop, "hero actions must not overlap the road");
			assert.ok(state.pageWidth <= width, "the gallery must not cause page overflow");
	...
	await t.test("without the enhancement script the fallback stays uncovered", async () => {
		await page.route("**/js/script.js", route => route.abort());
		await page.reload();
		assert.equal(await page.locator(".road-loader").isVisible(), false);
		assert.equal(await page.locator(".road-carousel-group").count(), 1);
		assert.equal(await page.locator(".road-photo img").first().isVisible(), true);
	});
});
