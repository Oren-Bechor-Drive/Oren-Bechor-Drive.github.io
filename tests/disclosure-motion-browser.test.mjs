...
test("mobile disclosures animate pointer input without delaying keyboard or focus", async (t) => {
	...
	for (const [triggerSelector, surfaceSelector] of [
		...
		await t.test(surfaceSelector, async () => {
			...
			assert.ok(opening.includes("opacity"), "pointer opening must bridge the visibility change");
			assert.equal(await surface.evaluate(element => element.inert), false);
			// Pausing halfway makes the interruption deterministic, without wall-clock sleeps.
			await surface.evaluate(element => {
				element.getAnimations().forEach(animation => { animation.currentTime = 70; });
			});
			await trigger.click();
			assert.equal(await surface.evaluate(element => element.inert), true,
				"a fading menu must stop accepting input immediately");
			assert.ok(await surface.evaluate(element => element.getAnimations().some(animation => animation.transitionProperty === "opacity")),
			...
			assert.equal(await surface.evaluate(element => element.getAnimations().length), 0,
				"keyboard opening must be immediate");
			await page.keyboard.press("Escape");
			await trigger.hover();
			await page.mouse.down();
			assert.ok(await trigger.evaluate(element => element.getAnimations().length > 0),
	...
	assert.equal(await page.locator(".topic-options").evaluate(element => getComputedStyle(element).transform), "none");
	assert.equal(await page.locator(".topic-options").evaluate(element => getComputedStyle(element).transitionDuration.split(",")[0].trim()), "0.08s");
	await page.keyboard.press("Escape");
	await page.locator(".topic-select").hover();
	await page.mouse.down();
	const reducedPress = await page.locator(".topic-select").evaluate(element => {
		element.getAnimations().forEach(animation => animation.finish());
		return { transform: getComputedStyle(element).transform, opacity: getComputedStyle(element).opacity };
	});
	...
	for (const [triggerSelector, surfaceSelector] of [
		...
		await page.locator(triggerSelector).click();
		assert.ok(await page.locator(surfaceSelector).evaluate(element =>
			element.inert && element.getAnimations().some(animation => animation.transitionProperty === "opacity")),
	...
	await page.waitForFunction(() => !document.querySelector(".site-menu").inert);
	assert.equal(await page.locator(".site-menu").isVisible(), true);
	assert.equal(await page.locator(".site-menu").evaluate(element => element.inert), false);
...
test("touch selection closes the list immediately and canceled presses release feedback", async (t) => {
	...
	assert.equal(await page.locator(".topic-options").evaluate(element => element.inert), true);
	assert.equal(await trigger.textContent(), await page.locator(".topic-card").nth(1).textContent().then(text => text.trim()));
	assert.equal(await trigger.evaluate(element => element === document.activeElement), true);
