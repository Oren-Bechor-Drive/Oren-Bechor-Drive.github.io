...
Add one Node.js HTTP service alongside the unchanged no-build static site. It serves an explicit public-file allowlist and `/api/account/*` from one origin. GitHub Pages continues to serve public files; it cannot run the gateway. Account submission fails closed with a readable Hebrew status when the API is absent. Fields remain editable during initialization and service failures; entering text does not require an active service.
...
- The account page shows verified email and offers sign-out and a link to the public course preview. It does not promise protected lessons or saved progress from the static files.
