...
| `server/test-lessons.mjs` | Fixed synthetic section IDs and access levels. No lesson bodies or entitlement authority. |
...
The gateway passes opaque browser tokens and validated inputs to the learner-account module. That module returns safe response data, fixed redirect destinations and optional cookie instructions. Records, provider tokens, locks and the generation used to invalidate in-flight authentication remain private. The gateway does not coordinate record mutation or await ordering. Its lesson route calls `readLesson`, which requires an ordinary authenticated session, serializes refresh/read with session mutations, and returns selected fields from the learner-scoped `read_section` RPC. The existing Supabase adapter and deterministic test adapter justify the provider seam; no persistence adapter is introduced while only local memory storage exists.

`account/test-lessons.html` contains descriptions and navigation, with an empty reading area. Its module loads only the selected test lesson, renders text without HTML interpretation, and clears/revalidates it when leaving or restoring the tab. It has no progress persistence. Development operators publish the synthetic bodies and grant temporary paid access through the SQL files in `supabase/development/`; the gateway never imports or executes those files. See [setup and API contract](test-lessons.md).

`tests/helpers/test-lessons.mjs` owns deterministic Auth, a disposable PostgreSQL instance, synthetic lesson setup and their shared lifetime. Its callers receive only `origin`, `grant(email, until?)`, `revoke(email)` and `close()`. Identity and session creation complete before the deterministic provider records tokens; simultaneous logins for one email share one fixture identity. Each grant owns its connection, settings and rollback, so a rejected or concurrent grant cannot corrupt another scenario. Revocation affects only that learner's development grant, and repeatable cleanup closes both the application and database.

Gateway and Chromium tests arrange access through this fixture and observe HTTP or rendered lesson results. They use real learner roles, RLS and the existing `read_section` function for free/paid decisions. SQL script idempotency is checked separately in `tests/database/test-lessons.test.mjs`; direct entitlement write denial stays in `tests/database/foundation.test.mjs`. The REST adapter test verifies learner tokens and RPC arguments. These seams do not claim to test live Supabase Auth or hosted PostgREST end to end.
