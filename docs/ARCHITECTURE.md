...
The course library links to independently authored learning pages. Each learning section owns a stable anchor, a declared heading, explanations, video placeholders and a link to its own practice-quiz page. Each quiz owns its title, questions and return links. `js/quiz.js` enhances that HTML with question navigation and answer review; the baseline stays usable when JavaScript is unavailable.

`scripts/learning-content.mjs` owns development-time interpretation through `readLearningContent(rootDir)`. It discovers root-level HTML and returns plain `lessons`, `quizzes` and `issues`, not live DOM elements. It closes the documents after reading them. Section titles follow `aria-labelledby`, and the same interpreted relationships feed structural checks and browser journeys.
...
Discovery includes root and nested `.html`/`.htm` files. It excludes hidden entries, symlinks and these directories: `assets`, `css`, `js`, `docs`, `scripts`, `tools`, `tests`, `test`, `fixtures`, `node_modules`, `vendor`, `coverage`, `dist`, and `build`. Put learner-facing pages outside these directories. Learning-content verification currently expects learning and quiz pages at the repository root even though the media audit can inspect nested pages.
