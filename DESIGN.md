...
The hero heading, description, and buttons stay fully opaque throughout their 1s entrances with the strong `--ease-out` curve, arriving quickly and settling slowly. The heading starts immediately from off screen to the right, translated by `100vw`. After 325ms, the description and both buttons move together from below the viewport, translated by `100vh`. Keep the hero shell in its own stacking context at `z-index: 0`. The hero clips overflow so these entrances stay within the hero and do not create scrollbars. The text sequence finishes at 1.325s. Focusing a hero button immediately finishes the description and button entrances, preventing focus from scrolling the hero internally. Reduced motion shows all hero content immediately in its resting position.
...
The user selected option A, an open library organized by named learning topics. `course.html` implements that layout as a preview, with no fixed learning order, locked topics, or fabricated completion progress. Preserve the existing Varela Round font, blue actions, pale blue canvas, and white 16px-radius topic cards. The header is compact and stays in normal flow. Its clickable brand leads to `index.html`. The opposite side reserves a blank 44px profile slot, hidden from assistive technology and without a button or menu until profile actions exist. This header is shared visually by the library and learning page.

The reading order is a short introduction, search, an optional pale-blue return-to-last-topic panel, and the subject grid. The return panel appears only when this browser has a valid previous topic. Two columns become one below 640px, with a maximum content width of 1080px. Each native details/summary card contains a topic title, the welcome page's description, and a disclosure cue. Opening it exposes a compact outline from the supplied brief. All outlines are accessible without JavaScript. No illustration, animation, fake video player, or sample lesson count is needed for this layout.

Search and its recovery controls appear only after initialization; an announced result count and an empty state accompany filtering. Opening a remembered topic clears an incompatible search and focuses its summary. Keyboard focus uses the existing solid blue outline. The preview includes a preparation notice and noindex metadata. The right-of-way card includes a blue "ללמידה" link after expansion to `right-of-way.html`. Other topics remain outlines without inactive learning links. The preparation notice distinguishes the available reading draft from outstanding lessons and videos.

### First learning page

`right-of-way.html` presents the richest subject in the supplied PDF, pages 12-18. A back-to-library link and introduction precede a right-hand contents list and four separate white reading sections, each with its own border, rounded corners, and 28px separation. The four freely accessible sections are priority rules, left turns, right turns, and U-turns. The contents list is sticky on desktop and becomes a two-column list above the reading content below 901px. Use native anchor links and baseline HTML throughout; this page needs no JavaScript. Short amber takeaways distinguish the key point in each section. Source labels and source links are omitted from the learning interface; provenance remains in README.md.

After each key point, reserve a compact, pale 80px-minimum-height video space with a dashed border and descriptive Hebrew text. These are accessible labeled placeholders, not broken images or inactive video players. Each section ends with a blue "לשאלון התרגול" link and a note identifying the 20 placeholder questions. All course preview pages remain noindex.
...
Each section links to its own static `quiz-<section-id>.html` page, with its title and lesson return links in the HTML. All quiz pages reuse `js/quiz.js`. The page opens the relevant sub-subject's quiz, using the same blank profile slot and home-link brand. A compact 820px-wide layout shows the subject title, placeholder notice, question count, direct question selector, one fieldset with four native radio choices, and previous/next controls. There are 20 placeholder questions per sub-subject, with every third question reserving a video space. Question content and choices are explicitly placeholders. Selection remains visible when returning to a question, and any question may be skipped or selected directly. Completion reports only how many questions have a selection, never a score or correctness. Review returns to the same answers. Without JavaScript or with its module blocked, all 20 question fieldsets remain available in the baseline HTML, and navigation back to the learning page still works.

### Content and verification constraints

The September 18 architecture changes preserve this layout, typography, motion and HTML formatting. Quiz forms marked `data-quiz-placeholder` retain the approved 20-question preview layout. The shared interaction follows authored question order and does not depend on numeric IDs or a fixed video position. Remove the placeholder marker only as part of an approved real-content update, including its visible notices.
