...
The reading order is a short introduction, search, an optional pale-blue return-to-last-topic panel, and the topic browser, within a maximum width of 1080px. The return panel appears only when this browser has a valid previous topic. At 900px and above, successful initialization presents a compact vertical topic list on the right and a separate white reading panel on the left. Selecting a topic highlights its row without rearranging the list. The list stays sticky while reading long outlines and scrolls internally if the viewport is too short. The panel shows the title, description, outline, availability note and any learning link. Its minimum height limits layout shifts between short topics. Content comes from the baseline HTML; no second authored copy is maintained.

Below 900px, or when JavaScript is unavailable, use a single column of compact native details/summary rows. Closed rows show the title and chevron; opening one reveals its description and outline. Topics share the native exclusive group `course-topics`; older browsers retain readable disclosures even without group support. The seven topics shared with the welcome page retain its descriptions. Selection carries across viewport changes, with focus transferred to the corresponding control when needed. No illustration, animation, fake video player, or sample lesson count is needed for this layout.

The library presents foundations, signs and speed, roads and lanes, right-of-way and turns, roundabouts and intersections, overtaking, trip planning, test preparation, and new-driver/licensing/points information in that order. This is a browsing order, not a prerequisite sequence; every topic remains freely accessible.

Search and its recovery controls appear only after initialization; an announced result count and an empty state accompany filtering. Search filters both presentations, selects a matching desktop outline when necessary, and hides the reading panel for empty results. Opening a remembered topic clears an incompatible search and focuses its visible control. Desktop selection uses a vertical tablist with Up/Down and Home/End navigation, one tab stop for the selected topic, and a labeled, focusable reading panel. Keyboard focus uses the existing solid blue outline. Default display and automatic search selection do not create remembered history; choosing a topic does. The preview includes a preparation notice and noindex metadata. The right-of-way outline includes a blue "ללמידה" link to `course/right-of-way/index.html`. Other topics remain outlines without inactive learning links. The preparation notice distinguishes the available reading draft from outstanding lessons and videos.

### First learning page

`course/right-of-way/index.html` presents the richest subject in the supplied PDF, pages 12-18. A back-to-library link and introduction precede a right-hand contents list and four separate white reading sections, each with its own border, rounded corners, and 28px separation. The four freely accessible sections are priority rules, left turns, right turns, and U-turns. The contents list is sticky on desktop and becomes a two-column list above the reading content below 901px. Use native anchor links and baseline HTML throughout; this page needs no JavaScript. Short amber takeaways distinguish the key point in each section. Source labels and source links are omitted from the learning interface; provenance remains in README.md.
