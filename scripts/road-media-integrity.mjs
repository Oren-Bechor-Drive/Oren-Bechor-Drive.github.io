...
export async function auditRoadMedia({ rootDir, htmlPath = "index.html" }) {
	...
	for (const image of document.querySelectorAll("img[data-road-media], [data-road-carousel] .road-car > img, [data-road-carousel] .road-photo img")) {
	...
	if (gallery) {
		await auditGallery(gallery, path.dirname(absoluteHtmlPath), issues, inspectSource);
	}
...
async function auditGallery(gallery, pageDir, issues, inspectSource) {
	async function filesIn(directory) {
		try {
			return (await readdir(path.join(pageDir, directory), { withFileTypes: true }))
	...
	const availableCars = (await filesIn(carDirectory)).filter(name => /^car-.*\.png$/.test(name));
	...
	const templates = cars.map(car =>
		cleanReference(car.querySelector(":scope > img")?.getAttribute("src") ?? ""));
	for (const name of availableCars) {
		const source = `${carDirectory}/${name}`;
		if (!templates.includes(source)) issues.push(`${source}: missing carousel template`);
	}
	for (const source of new Set(templates)) {
		if (!availableCars.some(name => source === `${carDirectory}/${name}`)) {
	...
	if (templates.length === 0) issues.push("Student gallery: no car templates");
	for (const car of cars) {
		if (!car.matches(".road-car") || car.querySelectorAll(":scope > img").length !== 1) {
			issues.push("Student gallery: each group child must be a car template with one sprite");
		}
		if (car.querySelectorAll(".road-photo img").length !== 1) {
			issues.push("Student gallery: each car template needs exactly one fallback photo");
		}
	}

	const photoDirectory = "assets/images/students-pass";
	const photoFiles = (await filesIn(photoDirectory)).filter(name => /\.png$/i.test(name));
	const numberedPhotos = [];
	for (const name of photoFiles) {
		if (!/^[1-9]\d*\.png$/.test(name)) {
			issues.push(`${photoDirectory}/${name}: filename must be a positive number followed by .png`);
	...
	for (const name of numberedPhotos) {
		...
		if (number !== expected) issues.push(`${photoDirectory}/${expected}.png: numbering gap; discovery stops before ${name}`);
		expected = number + 1;
		await inspectSource(`${photoDirectory}/${name}`);
	}
	if (numberedPhotos.length === 0) issues.push(`${photoDirectory}: no numbered student photos`);

	const fallback = cars.flatMap(car => [...car.querySelectorAll(".road-photo img")]);
	fallback.forEach((image, index) => {
		const expectedSource = `${photoDirectory}/${index + 1}.png`;
		if (cleanReference(image.getAttribute("src") ?? "") !== expectedSource) {
			issues.push(`Student gallery fallback ${index + 1}: expected ${expectedSource}`);
		}
	});
}
