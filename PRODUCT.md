...
- Each sub-subject is a separate reading section. Media is selected by teaching purpose: video for movement and timing, images or diagrams for recognition and layouts, and text alone for summaries and administrative explanations. The lessons reserve 16 videos and 12 images/diagrams; assets remain outstanding. Each of the ten learning topics has one 20-question multiple-choice quiz preview at `course/<topic>/quiz/`. The right-of-way quiz covers all three turn sections. Questions and answers are placeholders. Quiz media will be chosen when scenarios are authored, without fixed video positions. Learners can navigate and revisit answers freely, but finishing requires all questions to be answered. Missing answers produce Hebrew feedback and focus the first unanswered question. Complete previews report answer count without grades, persistence or access restrictions. The four former quiz URLs remain noindex transition pages linking to the topic quiz and original reading anchor. Source labels are omitted from the learning interface and preserved in maintenance documentation.
...
- Two synthetic test lessons at `account/test-lessons.html` exercise account-only free access and paid test access through the local gateway and the existing learner-scoped database function. Bodies load only after server-side checks. A trusted development SQL operation grants temporary access; the browser cannot purchase or grant it. These are test texts with no driving instruction. Learners can explicitly save a reading percentage, restore it in another signed-in browser, and resolve stale-save conflicts before continuing. Local browser/API tests prove expiry blocks paid reads and saves while retaining progress, renewal restores access, and learners remain isolated, including a simulated 31-day lapse. This is a development-entitlement simulation, without billing or a cancellation endpoint. See [local test lessons](docs/test-lessons.md) and the [manual walkthrough](docs/manual-test-lessons.md).
...
The [paid-release scope](docs/plans/paid-release-scope.md) records the owner-approved target: all ten topics and their supplied media, ten graded quizzes, saved learning progress, paid content protection and a monthly subscription with a once-only three-day trial. Passing at 17/20 permits optional topic completion. After ten days without renewal following subscription expiry, all saved learning progress except completed topics is deleted. These service capabilities are not implemented by the static quiz milestone. Price, content delivery, business/support details, provider selection and policy review remain launch dependencies.
